...
// with modifications
  ...
  liquidmon.timeSeriesChart = function () {
  ...
  liquidmon.multiTimeSeriesChart = function () {
    var margin = {top: 20, right: 20, bottom: 30, left: 50},
    legendMargin = {left: 20, top: 20},
    width = 880,
    legendWidth = 120
    innerWidth = width - legendWidth,
    height = 120,
    xValue = function(d) { return d[0]; },
    multiYValues = {"value": function(d) { return d[1]; }},
    xScale = d3.time.scale(),
    yScale = d3.scale.linear(),
    yTicks = 8,
    yFormat = function(d) { return yScale.tickFormat(yTicks, ".1p")(d/100.0); },
    ...
    yAxis = d3.svg.axis().scale(yScale).orient("left")
      .tickSize(3, 1).ticks(yTicks).tickFormat(yFormat),
    line = d3.svg.line().x(X).y(Y);
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        // lineDatas = [
        var lineDatas = [];
        Object.keys(multiYValues).map(function(multiYValuesKey) {
          var yValue = multiYValues[multiYValuesKey];
          var localData = data.map(function(d, i) {
            return [xValue.call(data, d, i), yValue.call(data, d, i), multiYValuesKey];
          });
          lineDatas.push(localData);
        });

        // Update the x-scale.
        xScale
          .domain(d3.extent(lineDatas[0], function(d) { return d[0]; }))
          .range([0, innerWidth - margin.left - margin.right]);

        // Update the y-scale.
        var yMax = d3.max(lineDatas.map(function(d,i) {
        ...
        // Otherwise, create the skeletal chart.
        var gEnter = svg.enter().append("svg").append("g");
        var gLegends = gEnter.append("g").attr("class", "legends");
        var gLines = gEnter.append("g").attr("class", "lines");
        gEnter.append("g").attr("class", "x axis");
        gEnter.append("g").attr("class", "y axis");
        ...
        // Update the legend
        var gLegends = gLegends.selectAll(".legend")
          .data(lineDatas)
          .enter()
          .append("g")
          .attr("class", function(d) {
          ...
          .attr("transform", function(d,i) {
            var dx = i * legendMargin.top;
            return "translate(" + (innerWidth - margin.left - margin.right + legendMargin.left) + "," + dx + ")";
          });

        var colors = gLegends.append("rect")
          .attr("class", "color")
          .attr("width", 10)
          .attr("height", 10);

        var text = gLegends.append("text")
          .attr("dx", 15)
          .attr("dy", 10)
          .style("pointer-events", "none")
        ...
        // Add mouse event handlers for focus
        svg.on("mouseover", function() {
          focus.style("display", "inherit");
        }).on("mouseout", function() {
          focus.style("display", "none")
          ...
          var mouse = d3.mouse(this);
          mouse[0] = mouse[0] - margin.left;
          mouse[1] = mouse[1] - margin.top;
          var timestamp = xScale.invert(mouse[0]);
          var mouseYVal = yScale.invert(mouse[1]);
          var index = bisect(lineDatas[0], timestamp);

          var startDatum = lineDatas[0][index - 1],
          endDatum = lineDatas[0][index];
          ...
          if (!startDatum) {
            ...
            idx = (timestamp - startDatum[0]) > (endDatum[0] - timestamp)
              ? index
              : index - 1;
          }

          var values = lineDatas.map(function(d, i) {
            return d[idx][1];
          }).sort();
          var valIndex = bisectValues(values, mouseYVal);
          var upper;
          var lower;
          lineDatas.map(function(d, i) {
            if (d[idx][1] === values[valIndex]) {
              upper = d;
            } else if (d[idx][1] === values[valIndex-1]) {
              lower = d;
            }
          });
          var datum;
          ...
          if (!upper) {
            ...
            var startDatum = lower[idx];
            var endDatum = upper[idx];

            datum = (mouseYVal - startDatum[1]) > (endDatum[1] - mouseYVal)
              ? endDatum
              : startDatum;
          }
          ...
          if (datum) {
            ...
            var magicFuckingNumber = 180 + legendWidth;
            if (cx + magicFuckingNumber > width) {
              tooltip.attr("dx", tooltipOffsetXLeft);
            } else {
              tooltip.attr("dx", tooltipOffsetXRight);
            ...
            var year = datum[0].getFullYear();
            var value = tooltipFormat(datum[1]);
            var lineName = datum[2];
            marker.attr("class", "marker " + lineName);
    ...
    chart.ys = function(_) {
      if (!arguments.length) return multiYValues;
      multiYValues = _;
      return chart;
    };
